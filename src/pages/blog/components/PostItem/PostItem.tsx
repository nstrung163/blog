...
interface PostItemProps {
  ...
  handelStarEditing: (postId: string) => void
}

export default function PostItem({ post, handleDelete, handelStarEditing }: PostItemProps) {
  return (
    <div>
      <div className='flex flex-col items-center overflow-hidden rounded-lg border md:flex-row'>
        <div className='group relative block h-48 w-full shrink-0 self-start overflow-hidden bg-gray-100 md:w-32 lg:w-48'>
          <img
          ...
          <div>
            <div className='inline-flex rounded-md shadow-sm' role='group'>
              <button
                ...
                onClick={() => handelStarEditing(post.id)}
              >
                Edit
              </button>
              <button
